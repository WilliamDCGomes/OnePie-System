...
public class EditarVenda extends javax.swing.JFrame {
    ...
    private void apagar(){
        int confirma = JOptionPane.showConfirmDialog(null, "TEM CERTEZA QUE DESEJA APAGAR ESTA VENDA?","ATENÇÃO",JOptionPane.YES_NO_OPTION);
    ...
    private void alterar(){
        String sql = "update vendas_ml set CÓDIGO=?,DATA=?,QUANTIDADE_VENDIDA=?,NOME=?,PREÇO=?,PORCENTAGEM=?,TARIFA_ML=?,FRETE=?,VALOR_LÍQUIDO_DO_ML=?,CUSTO=?,LUCRO=?,OBSERVAÇÕES=? where CÓDIGO_DO_CLIENTE=?";
        try {
            pst=conexao.prepareStatement(sql);
            pst.setString(1,codigoPeca.getText());
            pst.setString(2, dataVenda.getText());
            pst.setInt(3, Integer.parseInt(quantidadeVendida.getText()));
            ...
            pst.setString(12,cadastroObservacoes.getText());
            pst.setString(13,codigoCliente.getText());
            pst.executeUpdate();
